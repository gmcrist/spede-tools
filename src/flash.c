...
/* $Header: /export/home/aleks/Projects/Intel-159/Samples/00-Tools/flash/source/RCS/flash.c,v 1.3
 * 2001/04/23 02:41:13 aleks Exp $ */
        ...
        if (argv[j][0] == '-') {
            switch (argv[j][1]) {
                ...
                case 'f':
                    ...
                    strncpy(cfg.download_file, argv[argc - 1], sizeof(cfg.download_file) - 2);
                    goto DONE_PARSING;
...
DONE_PARSING:
    ...
    if (generate_config_file_flag) {
         ...
         */
        PRINT_CFG(&cfg);
    } else if (debugger_only_flag || emacs_debugger_flag) {
        if (cfg.download_file[0] == 0) {
...
void config_defaults(cfg) CONFIG *cfg;
{
...
void OK_to_be_here(diag) FILE *diag;
{
#ifdef USE_UTMPX
    struct utmpx *pxusr;
    pid_t         me = getpid();
    ...
    while ((pxusr = getutxent()) != NULL) {
        if (pxusr->ut_pid == me && strcmp(pxusr->ut_host, DEC_SPL_STRING)) {
