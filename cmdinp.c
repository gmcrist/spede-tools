...
/* $Header: /export/home/aleks/Projects/Intel-159/Core/00-Tools/flash/source/RCS/cmdinp.c,v 1.11 2001/08/31 05:50:23 aleks Exp $ */
...
CONFIG *cfg;
{
	...
	for(i = 0; i < NUMBER_ALL_CMDS;i++,cmdtbl++) {
		if((strncmp(cmdtbl->command,cmd_words[0],
				strlen(cmd_words[0]))) == 0){
			(cmdtbl->func)(cfg);
			done = 1;
			break;
		}
...
void
print_args(int cnt, char args[10][512])
{
...
CONFIG *cfg;
{
  ...
  int  j;
	...
	if(access(gdb_init_fname,F_OK) < 0) {
		...
		sprintf( temp_line, "cp " MODEL_GDBINIT_FILE " %s",
			 gdb_init_fname );
		if( verbose_flag ) {
			printf( "%% %s\n", temp_line );
		}
 ...
 status = 0;
	...
	if( status == 0 ) {
		...
		execlp(cfg->gdbpath,args[0],args[1],args[2],args[3],
			args[4],
...
CONFIG *cfg;
{
	...
	if( arg_count == 2 && cmd_words[1][0] == 'f' ) {
		/* There is another argument check it, see if it is 'f' */
	    cmd = "o";
	}
	else {
...
CONFIG *cfg;
{
     ...
     */
    if( strchr( cmd_words[1], ';' ) ||
	strstr( cmd_words[1], "IFS" ) ||
	strstr( cmd_words[1], "PATH" ) ) {
...
int
target_readline( int targ, char * buffer, int size_buffer )
{
...
CONFIG *cfg;
{
  ...
  FULL_ERROR_CLEANUP :
	...
	printf("\b%c", spinner[si++]); fflush(stdout);
	if( si >= sizeof(spinner) ) { si = 0; }
